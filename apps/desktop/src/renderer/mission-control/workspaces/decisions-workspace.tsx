...
function proposalAuthorizationLabel(proposal: AnalysisProposalSnapshotRecord): string {
  const human = proposal.authorization.human.eligible
    ? '人工可授权'
    : `人工阻断：${proposal.authorization.human.blockers.join(' · ')}`;
  const policy = proposal.authorization.policy.eligible
    ? '策略内自动可授权'
    : `策略阻断：${proposal.authorization.policy.blockers.join(' · ')}`;
  return `${human} · ${policy}`;
}
  ...
  const authorizeSelectedBatch = async () => {
    ...
    try {
      ...
      try {
        ...
        refreshWarning = ' Decision 列表刷新失败，请手动刷新；不要重复授权。';
      }
      if (authorityRef.current !== capturedKey || mutationSequence.current !== current) return;
      setFeedback(`${result.mode === 'policy_auto' ? '策略自动' : '人工审批'}已签发同一 MissionGrant：${result.proposalIds.length} 条建议整批授权，尚未执行 Ads。${refreshWarning}`);
    ...
    <PageFrame className="decision-domain-page" pageId={view.replace('/', '-')} title="建议与审批" description={copy.description} task={<TaskBanner compact eyebrow="CRUX DECISIONS" title={copy.task} description={copy.taskDescription} primaryAction={{ actionId: 'decisions.recommendations.create', label: '新建 Decision', disabled: view !== 'decisions/recommendations' || !can('decisions.recommendations.create') || busy, disabledReason: blockedReason, onClick: () => storeContext && setEditor({ record: null, draft: decisionDraft(storeContext) }) }} secondaryActions={onInspectBoundary ? [{ actionId: 'decision-boundary', label: '查看接入边界', onClick: onInspectBoundary }] : []} status={<span className="decision-domain-authority" data-state={viewReady ? expectedState : 'BLOCKED'}>{previewMode ? '仅开发预览' : viewReady ? '生产 Authority' : '已阻断'}</span>}>{previewMode && <p className="decision-domain-preview-note">显式内存 adapter · Amazon US · USD · 批准不代表执行</p>}</TaskBanner>} summary={<SummaryStrip ariaLabel="Decision 当前范围" items={[{ id: 'store', label: '店铺数据域', value: storeContext ? String(storeContext.storeId) : '等待 Main' }, { id: 'view', label: '当前队列', value: copy.title }, { id: 'count', label: '匹配 Decision', value: `${filtered.length} 条` }, { id: 'currency', label: '站点 / 币种', value: 'US / USD' }]} />}>
      <div className="decision-domain-layout">
        <WorkbenchPanel className="decision-domain-list-panel" title={copy.title} description="视图默认按决策状态分流。" footer={`第 ${safePage}/${pageCount} 页 · ${filtered.length} 条`} toolbar={<button className="workspace-button workspace-button--primary" disabled={view !== 'decisions/recommendations' || !can('decisions.recommendations.create') || busy} onClick={() => storeContext && setEditor({ record: null, draft: decisionDraft(storeContext) })} type="button"><Plus size={15} />新建</button>}>
          {view === 'decisions/decided' && <section className="decision-domain-batch-authority decision-domain-analysis-batch" aria-label="分析建议整批授权"><header><div><strong>整批授权一次</strong><small>先选 Mission；系统只暴露它的最新不可变 action batch，无需选中某条已决策记录。Main 校验后，人工审批与策略自动签发同一种 MissionGrant。</small></div><button className="workspace-button workspace-button--primary" disabled={!canAuthorizeAnalysisBatch || authorizationProposals.length === 0 || busy || authorizationAnalysisLoading} onClick={() => void authorizeSelectedBatch()} type="button"><ShieldCheck size={14} />{pending === 'analysis-authorize' ? '授权校验中…' : `授权 ${authorizationProposals.length} 条`}</button></header><div className="decision-domain-analysis-scope"><label><span>授权 Mission</span><select aria-label="选择授权 Mission" disabled={busy || authorizationMissions.length === 0} value={authorizationMissionId} onChange={(event) => setAuthorizationMissionId(event.target.value)}><option value="">{authorizationMissions.length ? '请选择 Mission' : '暂无可授权 Mission'}</option>{authorizationMissions.map((missionId) => <option key={missionId} value={missionId}>{missionId}</option>)}</select></label><label><span>最新动作批次</span><select aria-label="选择最新动作批次" disabled={busy || authorizationAnalysisLoading || authorizationBatches.length === 0} value={authorizationBatchId} onChange={(event) => setAuthorizationBatchId(event.target.value)}><option value="">{authorizationAnalysisLoading ? '正在读取动作批次…' : authorizationBatches.length ? '等待最新批次' : '暂无动作批次'}</option>{authorizationBatches.map((batch) => <option key={batch.id} value={batch.id}>{batch.id} · {batch.proposalCount} 条</option>)}</select></label></div>{authorizationAnalysisError ? <p className="decision-domain-analysis-error">{authorizationAnalysisError}</p> : authorizationAnalysisLoading ? <p>正在读取所选 Mission 的分析建议…</p> : authorizationProposals.length ? <div className="decision-domain-analysis-proposals">{authorizationProposals.map((proposal) => <article key={proposal.id}><span><b>{proposal.entityName}</b><small>{proposal.source === 'rule_ai' ? '规则 + AI 一致' : proposal.source} · r{proposal.adEntityRevision ?? '—'}</small></span><strong>${(proposal.currentBidCents / 100).toFixed(2)} → ${(proposal.proposedBidCents / 100).toFixed(2)}</strong><em data-eligible={(proposal.authorization.human.eligible || proposal.authorization.policy.eligible) || undefined}>{proposalAuthorizationLabel(proposal)}</em></article>)}</div> : <p>请选择包含分析建议的 Mission；策略内自动可以直接校验待审批或待复核 Decision。</p>}</section>}
        ...
        <div className="decision-domain-detail">{selected ? <>
          ...
          <section className="decision-domain-grants"><header><div><h3>MissionGrant</h3><p>人工审批与策略自动共用同一不可变授权模型；终态从持久化 grant events 派生。</p></div><ShieldCheck size={19} /></header>{grants.length ? <div>{grants.map((grant) => { const terminal = terminalGrantEvent(grant); return <article data-revoked={terminal?.eventType === 'revoked' || undefined} data-terminal={terminal?.eventType} key={grant.id}><div><strong>{grant.id}</strong><small>{grant.issuer.type === 'policy' ? '策略自动' : '人工审批'} · {grant.allowedAdEntityIds.length} 个对象 · ≤ {grant.maxChangePct}% · ${grant.totalImpactBudget} · {terminal ? `已${terminal.eventType}` : '有效'}</small></div>{view === 'decisions/decided' && grant.issuer.type === 'human' && <button className="workspace-button workspace-button--secondary" disabled={!can('decisions.grants.revoke') || busy || Boolean(terminal)} onClick={() => void revokeGrant(grant)} type="button">{terminal ? terminal.eventType === 'revoked' ? '已撤销' : terminal.eventType === 'consumed' ? '已消费' : '已过期' : '撤销人工授权'}</button>}</article>; })}</div> : <p>当前 Mission 没有授权。</p>}</section>
