...
**DELIVERY: APP_READY.** 2026-06-23 Windows 桌面 high-fidelity UI 重构已完成，并重新跑通 Windows installer/portable 打包、packaged launch smoke、manifest-driven final-readiness、READY bundle 和 READY safety。权威 final-readiness 为 `output\codex-evidence\final-readiness-20260623173135.json`，证据选择 manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260623173135.json`，package launch smoke 为 `output\codex-evidence\package-launch-smoke-1782206620951.json`，READY bundle 为 `output\delivery-bundles\v15-delivery-bundle-20260623173135-ready`。本轮 UI 将主窗口改为更清晰的业务分组导航、紧凑状态标签、表格化录入、AI 输出合同标签和更少首屏长文案。应用内广告执行仍保持 fail-closed，不做批量自动写入；后续每个广告动作必须绑定自己的店铺、站点、广告组合、广告活动、广告组、ASIN、对象和动作，并独立审批、截图和回读。
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782205829469.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `89948964`，SHA-256 `2955BB05ADAD62B5DF7F891B4FAE352EC30801F5C87716E8D89CA62D6853C5A8`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `89783210`，SHA-256 `9437D669A2C1AA0973FC588569396A5BB38A6CD75F18C61A18C57A1A896D6071` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782206620951.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260623173135.json` 为 2026-06-23 high-fidelity UI 重构后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260623173135.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260623173135.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260623173135-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260623173135-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260623173135.json`、`output\codex-evidence\package-launch-smoke-1782206620951.json`、`output\delivery-bundles\v15-delivery-bundle-20260623173135-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
