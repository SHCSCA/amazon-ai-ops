...
| 广告量化指标聚焦 | 已接入 | `广告量化` 首屏指标卡支持点击聚焦：全部对象、浪费超支、高 ACOS、出单对象、可扩量、待复核。聚焦后非激活指标卡降到 60% 透明度并在 hover/focus 恢复，避免首屏指标全部抢注意力；下方实体诊断表、产品/广告对象阶段时间线和复核队列同步显示 `visible/total`，并在 `aria-live` 行说明只改变本页视图，不改变建议状态、不审批、不写入 Ads。 |
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782465781877.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83119892`，SHA-256 `C64D08D5F46BB090E2B15F11358F44A0320288293EF97446BEEA84547E33A5D0`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82954143`，SHA-256 `692DFC1EC43114A6C48AA5157E0C60EA0A45710271E8C8E614AA367E78F8106C` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782465872048.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260626172416.json` 为 2026-06-26 high-fidelity UI / AI/import feedback / credential refresh / 产品配置批量目标 ACOS / 广告量化指标聚焦降噪 / 今日看板主行动反馈、关键词机会排序与筛选反馈后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260626172416.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260626172416.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260626172416-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260626172416-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| `output/codex-evidence/current-business-ui-smoke-1782465781877.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260626172416.json`、`output\codex-evidence\package-launch-smoke-1782465872048.json`、`output\delivery-bundles\v15-delivery-bundle-20260626172416-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
