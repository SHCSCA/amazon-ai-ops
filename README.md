...
**DELIVERY: APP_READY.** 2026-06-24 Windows 桌面 high-fidelity UI 与 AI 输出合同硬化已完成，并重新跑通 Windows installer/portable 打包、packaged launch smoke、manifest-driven final-readiness、READY bundle 和 READY safety。权威 final-readiness 为 `output\codex-evidence\final-readiness-20260624090830.json`，证据选择 manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260624090830.json`，package launch smoke 为 `output\codex-evidence\package-launch-smoke-1782263264718.json`，READY bundle 为 `output\delivery-bundles\v15-delivery-bundle-20260624090830-ready`。本轮 UI 将主窗口改为更清晰的业务分组导航、紧凑状态标签、表格化录入、AI 输出合同标签和更少首屏长文案；AI 设置会把旧的低 `maxTokens` 提升到结构化输出下限，并把广告策略诊断的证据引用归一化到当前 evidencePack 中真实存在的 ID。应用内广告执行仍保持 fail-closed，不做批量自动写入；后续每个广告动作必须绑定自己的店铺、站点、广告组合、广告活动、广告组、ASIN、对象和动作，并独立审批、截图和回读。
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782263174619.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83054292`，SHA-256 `7F91C2A3C2812DDDFDE76FB09E5E508D2B6F6FAFDF94F719C30D293779EBF788`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82888546`，SHA-256 `C8531FB7744551E5A57615AE8FE58E90904E9D0F0D918E2CEB15F2A15EC3218C` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782263264718.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260624090830.json` 为 2026-06-24 high-fidelity UI / AI contract refresh 后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260624090830.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260624090830.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260624090830-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260624090830-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| `output/codex-evidence/current-business-ui-smoke-1782263174619.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260624090830.json`、`output\codex-evidence\package-launch-smoke-1782263264718.json`、`output\delivery-bundles\v15-delivery-bundle-20260624090830-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
