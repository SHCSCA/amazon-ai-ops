...
| 运营事件任务反馈 | 已接入 | `运营事件` 页面现在把“记录 BD/Coupon/调价/库存/Listing 变化”作为首屏任务，主按钮使用 OperatorTaskPanel busy/disabled/spinner 合同，底部内联保存按钮改为 `保存到上下文` 以避免重复主行动；提交后表单立即清空并回弹，页面提示正在写入本地上下文，保存失败会恢复刚才填写的草稿，保存成功后最新事件卡片短暂高亮。 |
| 本地测试 | 增量通过 | 本轮重点测试：账号密码保存、工作范围首屏保存反馈、页面内范围表单字段确认、产品管理、产品配置成本回填/上下键微调、产品级看板、AI 阻断提示、广告量化、导入反馈、产品上下文和业务 UI smoke 通过；READY/final-readiness 脚本和 package launch smoke 通过 |
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782461034224.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83118455`，SHA-256 `76F2D0CD8466D7DB87A68FE004A0426B0E65E21B7B1ABCFED990808D73EFAD3C`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82952705`，SHA-256 `BA07C66009B32B42986D75714C93A2DF95E83672F67C26298D11EEC5F6471E1C` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782461110147.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260626160541.json` 为 2026-06-26 high-fidelity UI / AI/import feedback / credential refresh / 今日看板主行动反馈、关键词机会排序与筛选反馈后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260626160541.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260626160541.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260626160541-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260626160541-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 产品管理 | 产品列表、当前产品范围、产品/全局/广告对象时间线和跨页面入口已接入；工作范围页提供首屏确认范围任务和保存反馈；选择产品会更新 `scope.asin`，广告量化、优化建议、运营事件、关键词机会和 Listing 沿用同一产品上下文；产品配置页提供首屏保存目标任务、上/下方向键微调、实时健康度芯片和失焦/回车即时保存反馈，运营事件页提供首屏记录任务、提交即清空回弹、失败恢复草稿和保存后的新卡片高亮反馈 |
...
| `output/codex-evidence/current-business-ui-smoke-1782461034224.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260626160541.json`、`output\codex-evidence\package-launch-smoke-1782461110147.json`、`output\delivery-bundles\v15-delivery-bundle-20260626160541-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
pnpm run write:v15-evidence-manifest -- --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260626160541.json
pnpm run verify:v15-delivery
pnpm run verify:v15-final-readiness -- --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260626160541.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782461110147.json --out output\codex-evidence\final-readiness-20260626160541.json
# 先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，再导出交付包；导出器会拒绝 IN_PROGRESS README。
pnpm run export:v15-delivery-bundle -- --final-readiness output\codex-evidence\final-readiness-20260626160541.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260626160541-ready
pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260626160541.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260626160541-ready\delivery-bundle-manifest.json
```
