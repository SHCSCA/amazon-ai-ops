...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782703485265.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83120619`，SHA-256 `625CA199821F47599C36C24107208DE72C554EBC54941831ABCD0F7043D9F8D5`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82954869`，SHA-256 `5768F01E3D7B62C699D2D49CA6197C24DF6C9DE95AECF3CBF4004B0112DEFB3E` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782703552957.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260629112616.json` 为 2026-06-29 high-fidelity UI / AI/import feedback / credential refresh / 产品配置批量目标 ACOS / 数据采集动作按钮 busy 反馈 / 广告量化指标聚焦降噪 / 今日看板主行动反馈、关键词机会排序与筛选反馈 / ProgressiveDetails 折叠反馈 / 全局字体栈和表格行高契约后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260629112616.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-20260629112616.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260629112616-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260629112616-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| `output/codex-evidence/current-business-ui-smoke-1782703485265.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260629112616.json`、`output\codex-evidence\package-launch-smoke-1782703552957.json`、`output\delivery-bundles\v15-delivery-bundle-20260629112616-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
