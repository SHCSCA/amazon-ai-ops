...
**DELIVERY BASELINE: APP_READY.** 2026-06-22 Windows 桌面 packaged baseline 已完成 UX 降噪、主流程业务文案替换、Windows installer/portable 重新打包、packaged launch smoke、manifest-driven final-readiness 和 READY safety。该 baseline 的权威 final-readiness 为 `output\codex-evidence\final-readiness-20260618170712.json`，并已把 `output\codex-evidence\package-launch-smoke-1782118074963.json` 作为硬门槛，覆盖 `win-unpacked` 与 no-install portable 启动。2026-06-23 之后当前分支又追加了 post-READY renderer UX polish：AI 输出合同标签、广告量化输入检查标签化、Listing 手工录入表格化；这些源码改动已通过聚焦测试和 renderer smoke，但尚未重跑 `build:win`、`smoke:package-launch`、final-readiness、READY bundle 和 READY safety，因此不能把这些新增 UI 改动声明为刷新后的 APP_READY。应用内广告执行仍保持 fail-closed，不做批量自动写入；后续每个广告动作必须绑定自己的店铺、站点、广告组合、广告活动、广告组、ASIN、对象和动作，并独立审批、截图和回读。

| 项目 | 状态 | 证据/位置 |
|---|---:|---|
| v1.5 基线合并 | 已完成 | v1.5 业务后台重构和最终验收已合入 `master` |
| 本轮 packaged baseline 收尾改动 | 已完成 | UI 密度、长页面、业务化文案、无安装版启动 smoke、package rebuild hardening 和 2026-06-22 baseline APP_READY 聚合已接入 |
| Post-READY UX polish | 聚焦验证通过，最终交付链待刷新 | 设置页显示固定 AI 输出合同标签 `广告诊断 v1`、`广告解释 v1`、`Listing 草案 v1`、`异常回退规则`，人设字段只控制表达风格；`AI+规则建议输入检查` 默认展示紧凑指标标签并把依据折叠进 `输入明细和判断依据`；`手工录入当前 Listing` 改成 `基础信息`、`标题`、`五点`、`详情与搜索词` 表格化编辑器。验证：`pnpm exec vitest run apps\desktop\src\renderer\ai-output-contracts.test.ts apps\desktop\src\renderer\components\tag-metric-group.test.tsx apps\desktop\src\renderer\pages\settings-page.test.ts apps\desktop\src\renderer\pages\listing-optimization-page.test.ts`，`pnpm --filter @amazon-ai-ops/desktop run build:renderer`，`node scripts\smoke-business-ui-settings-delivery.js` -> `output\codex-evidence\business-ui-settings-delivery-smoke-1782121086227.json`，`node scripts\smoke-business-ui-data-pipeline.js` -> `output\codex-evidence\business-ui-data-pipeline-smoke-1782121097671.json`，`node scripts\smoke-business-ui-keyword-listing.js` -> `output\codex-evidence\business-ui-keyword-listing-smoke-1782121111129.json`，`pnpm run verify:ad-execution` |
| 本地测试 | 增量通过 | 本轮重点测试：11 个 renderer 测试文件 / 90 tests 通过，`scripts/package-scripts.test.mjs` 通过 |
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782117040397.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `89942987`，SHA-256 `E8738F8BA4818A0F8F0BE0FFC282CEFAFE52D40B46364F2F183AE1E3F61572BE`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `89777237`，SHA-256 `B0A3DB8D2EE8789FAC9C60859B358F3167F6EACE26599DB40F1E0A78885C8A3D` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782118074963.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | Baseline 通过；当前 post-READY UX polish 待刷新 | `output\codex-evidence\final-readiness-20260618170712.json` 为 2026-06-22 packaged baseline 的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260618170712.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛。当前分支新增 UX polish 后必须重跑最终交付链，才能声明这些新增源码对应的 APP_READY |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260618170712.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260618170712-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260618170712-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
| 交付证据脚本 | 通过 | 当前应用内执行回读 smoke 覆盖 session 创建、结构检查和中文现场证据未填写提示、打开 `session-input.json` 填写文件、打开 `session-input-guide.md` 填写说明、回读证据生成、回读证据 verifier 和路径展示：`output\codex-evidence\business-ui-ad-execution-smoke-1782117018229.json`；`交付验收` 页新增 `刷新最终验收` 和 `广告回读补证`，可从失败 readback gate 直接创建工作包、检查工作包、显示 `结构通过，现场证据待填写` 和中文缺失字段、生成回读证据、校验回读证据、用该证据刷新最终验收，并打开候选证据/工作包/操作清单/Ads UI 定位单/填写文件/填写说明，settings/delivery smoke 已覆盖：`output\codex-evidence\business-ui-settings-delivery-smoke-1782117033568.json`；当前 final-readiness、bundle export 和 READY safety 已刷新 |
...
| `output/codex-evidence/current-business-ui-smoke-1782117040397.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
| `output/codex-evidence/business-ui-shell-smoke-1782117000585.json` | 当前后台框架 smoke；确认左侧菜单按 `运营总览`、`数据与量化`、`广告执行`、`关键词与 Listing`、`系统与交付` 分组，旧 `v1.5 工作台` 不再作为总入口；`交付验收` 只汇总最终证据，不承载日常操作 |
| `output/codex-evidence/business-ui-data-pipeline-smoke-1782117007945.json` | 当前数据链路 smoke；确认 `数据采集` 和 `数据导入与校验` 分离，真实表格、导入指标、USD 口径、当前操作范围和失败动作指引可见 |
| `output/codex-evidence/business-ui-ad-execution-smoke-1782117018229.json` | 当前广告执行 smoke；覆盖优化建议、审批/回读路径、AI fallback 状态、证据不足阻断、fail-closed 执行边界，以及执行回读页可见的 session 工作包创建、结构检查、中文现场证据未填写提示、打开填写文件、打开填写说明、应用内生成回读证据、路径展示和隐藏技术区的 `prepare/verify/fill session` 命令入口 |
| `output/codex-evidence/business-ui-keyword-listing-smoke-1782117028225.json` | 当前关键词/Listing smoke；覆盖关键词机会、Listing 读取状态、ASIN/范围核对、本地草案生成，并确认草案生成提示区分 `AI 草案` 与 `规则 fallback 草案` |
| `output/codex-evidence/business-ui-settings-delivery-smoke-1782117033568.json` | 当前设置/交付 smoke；覆盖 AI 设置保存/测试/清除本地 Key、AI 调用审计、交付矩阵、非 READY 展示、`交付验收` 页应用内刷新最终验收，以及从失败 readback gate 创建/检查工作包、显示中文现场证据未填写字段、生成/校验回读证据、用生成的 PASS JSON 刷新最终验收，并打开候选证据、工作包目录、操作清单、Ads UI 定位单、填写文件和填写说明 |
| `output/codex-evidence/v15-listing-read-ui-smoke-1780991635633.png` | 历史 Listing 面板截图；当前完整 Listing 交互 smoke 以上方 `business-ui-keyword-listing-smoke-1782117028225.json` 为准 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260618170712.json`、`output\codex-evidence\package-launch-smoke-1782118074963.json`、`output\delivery-bundles\v15-delivery-bundle-20260618170712-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
当前报表采集交付边界是“真实 Ads 下载中心定位完成 + 启用后 page model 诊断通过 + 8/8 canary 通过 + full-8 真实下载 + manifest/DB/文件/验收审计一致”；Listing 内容边界现在是“手工录入/辅助读取后保存为本地版本，草案和覆盖分析只读取本地版本，不自动提交 Amazon”；AI 边界是“DeepSeek live、广告建议 AI 解释、Listing AI 草案三份证据均通过 verifier 且不泄露密钥，并且 AI 证据引用、输出 schema、正式动作准入和洞察分流均通过当前代码验证”；广告执行边界是“每个低风险人工 Ads UI 动作都有独立 approval、真实报表 sourceFiles/sourceRow、before、after、reload readback 和 `verify:ad-readback` PASS”。最终聚合必须先 `build:win` 生成当前代码的 installer 和 portable/no-install EXE，再用 `write:v15-evidence-manifest` 固定证据选择，并运行 `verify:v15-final-readiness -- --evidence-manifest <manifest> --package-launch-smoke <smoke>`；只有输出包含 `evidenceSelection.mode=manifest`、通过 `Release package hash` 与 `Package launch smoke` 的 final readiness JSON 才能用于交付包。导出 READY 交付包前，必须先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，这样包内文档和外部 READY safety 校验不会脱节；导出后再运行 `verify:v15-ready-safety`。应用内批量广告写入仍 fail-closed；2026-06-22 packaged baseline 已完成 UX 降噪、当前业务 UI smoke、Windows 打包、安装包 hash 记录、package launch smoke、manifest-driven final-readiness、READY bundle 和 READY safety，baseline READY 证据为 `output\codex-evidence\final-readiness-20260618170712.json` 与 `output\delivery-bundles\v15-delivery-bundle-20260618170712-ready`。2026-06-23 post-baseline UX polish 只做了聚焦 renderer/test/smoke 验证，必须重跑最终交付链后才能进入新的 READY 证据；`output\codex-evidence\final-readiness-2026-06-18-portable-fix.json` 与 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作为历史 READY 基线。
