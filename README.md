...
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83125050`，SHA-256 `F21EC053822B6C420FB3A79CCA80992DB7E36A42A7DDF1A6630D821F5DC6E344`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82959296`，SHA-256 `07A4EB2D539E1ED28591E04B1317DC43A9E9670F9D1EBB721FF47788711117B4` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782788680564.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260630110504.json` 为 2026-06-29 high-fidelity UI / AI/import feedback / credential refresh / 产品配置批量目标 ACOS / 数据采集动作按钮 busy 反馈 / 广告量化指标聚焦降噪与 AI 运行雷达反馈 / 今日看板主行动反馈、关键词机会排序与筛选反馈 / ProgressiveDetails 折叠反馈 / 全局字体栈和表格行高契约后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260630110504.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-20260630110504.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260630110504-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260630110504-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260630110504.json`、`output\codex-evidence\package-launch-smoke-1782788680564.json`、`output\delivery-bundles\v15-delivery-bundle-20260630110504-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
pnpm run write:v15-evidence-manifest --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260630110504.json
pnpm run verify:v15-delivery
pnpm run verify:v15-final-readiness --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260630110504.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782788680564.json --out output\codex-evidence\final-readiness-20260630110504.json
# 先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，再导出交付包；导出器会拒绝 IN_PROGRESS README。
pnpm run export:v15-delivery-bundle --final-readiness output\codex-evidence\final-readiness-20260630110504.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260630110504-ready
pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-20260630110504.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260630110504-ready\delivery-bundle-manifest.json
```
