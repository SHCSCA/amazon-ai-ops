...
| 审批中心盖章与队列反馈 | 已接入 | 批准、拒绝、阻断会在首屏显示 `SEALING` / `PASSED` / `REJECTED` / `BLOCKED` 类盖章式状态，按钮提交中会禁用并显示“正在批准/正在拒绝”。批准或拒绝成功后，当前行会按通过/拒绝色调滑出 180ms 并先从本地队列移除，随后刷新真实审批队列。三态决策按钮支持 hover/focus 组级弱淡化，当前按钮保持高亮，其余可用动作降到 40% 透明度，降低误点风险。该反馈只代表本地审批决策状态，不代表 Ads 已执行；真实广告动作仍必须进入人工执行、截图和回读 verifier。 |
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782463175585.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83118713`，SHA-256 `2F887DDD278428B6C3E63A7892AD744A72D0EF7B7EBFDAD98F4B93290656E8A7`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82952962`，SHA-256 `C1FBA55B52FFD457B66D9BC079BFF2F10FEF65536BFA271B246EB7B66FD1202E` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782463345838.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260626164257.json` 为 2026-06-26 high-fidelity UI / AI/import feedback / credential refresh / 今日看板主行动反馈、关键词机会排序与筛选反馈后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260626164257.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260626164257.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260626164257-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260626164257-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| `output/codex-evidence/current-business-ui-smoke-1782463175585.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260626164257.json`、`output\codex-evidence\package-launch-smoke-1782463345838.json`、`output\delivery-bundles\v15-delivery-bundle-20260626164257-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
