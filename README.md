...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782367917694.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83102567`，SHA-256 `457A002B9F8F48B44B5293AC9871F0E7DE6B535423B9D107A663028E43A621F4`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82936813`，SHA-256 `2055D97DCEAEFC575B479EF4A353FBB719812AD346D9295F67955B64B8CCC587` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782367994841.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260625141313.json` 为 2026-06-25 high-fidelity UI / AI/import feedback / credential refresh 后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260625141313.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260625141313.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260625141313-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260625141313-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 产品管理 | 产品列表、当前产品范围、产品/全局/广告对象时间线和跨页面入口已接入；选择产品会更新 `scope.asin`，广告量化、优化建议、运营事件、关键词机会和 Listing 沿用同一产品上下文；运营事件页提供首屏记录任务和保存后的新卡片高亮反馈 |
...
| `output/codex-evidence/current-business-ui-smoke-1782367917694.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| `output/codex-evidence/business-ui-data-pipeline-smoke-1782367890392.json` | 当前数据链路 smoke；确认 `数据采集` 和 `数据导入与校验` 分离，真实表格、导入指标、USD 口径、当前操作范围、失败动作指引和自动数据采集监控抽屉可见，且监控抽屉不会遮挡后续主流程按钮 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260625141313.json`、`output\codex-evidence\package-launch-smoke-1782367994841.json`、`output\delivery-bundles\v15-delivery-bundle-20260625141313-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
pnpm run write:v15-evidence-manifest -- --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260625141313.json
pnpm run verify:v15-delivery
pnpm run verify:v15-final-readiness -- --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260625141313.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782367994841.json --out output\codex-evidence\final-readiness-20260625141313.json
# 先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，再导出交付包；导出器会拒绝 IN_PROGRESS README。
pnpm run export:v15-delivery-bundle -- --final-readiness output\codex-evidence\final-readiness-20260625141313.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260625141313-ready
pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260625141313.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260625141313-ready\delivery-bundle-manifest.json
```
