...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782888591659.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `73974133`，SHA-256 `5D4118A36769C20EEF5F9A21C75601B6A90FB027EABCB0F919D20905DB371FB1`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `73808378`，SHA-256 `2B8C6835D5735F56535BC2D2F0701683491404C9B44811A9E86D9C7FA2F166BE` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782888667353.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-1782888756260.json` 为 2026-07-01 shared PageHeader semantic landmark / shared Panel named sections / VirtualDataTable 状态读回 / high-fidelity UI / AI-import feedback / data-import row-fade refresh / credential refresh / 产品配置载入编辑行确认反馈 / 产品配置批量目标 ACOS / 产品配置批量选择计数进度 / 数据采集动作按钮 busy 反馈 / Listing 本地动作按钮 busy 反馈 / 定时任务控制器按钮 busy 反馈 / 广告量化指标聚焦降噪与 AI 运行雷达反馈 / 今日看板主行动反馈 / 执行回读安全复选框确认反馈 / 执行回读证据链动作按钮 busy 反馈 / 执行回读本地路径打开 busy 反馈、共享 FormTable 固定反馈槽、关键词机会排序与筛选反馈 / 结构化筛选字段单元 / ProgressiveDetails 折叠反馈 / 全局字体栈和表格行高契约后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-1782888756260.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-1782888756260.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-2026-07-01T06-53-08-756Z-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-2026-07-01T06-53-08-756Z-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-1782888756260.json`、`output\codex-evidence\package-launch-smoke-1782888667353.json`、`output\delivery-bundles\v15-delivery-bundle-2026-07-01T06-53-08-756Z-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
