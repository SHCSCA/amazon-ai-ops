...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-1782879062227.json` 为 2026-07-01 high-fidelity UI / AI/import feedback / data-import row-fade refresh / credential refresh / 产品配置载入编辑行确认反馈 / 产品配置批量目标 ACOS / 产品配置批量选择计数进度 / 数据采集动作按钮 busy 反馈 / Listing 本地动作按钮 busy 反馈 / 定时任务控制器按钮 busy 反馈 / 广告量化指标聚焦降噪与 AI 运行雷达反馈 / 今日看板主行动反馈 / 执行回读安全复选框确认反馈 / 执行回读证据链动作按钮 busy 反馈 / 执行回读本地路径打开 busy 反馈、关键词机会排序与筛选反馈 / 结构化筛选字段单元 / ProgressiveDetails 折叠反馈 / 全局字体栈和表格行高契约后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-1782879016283.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-1782879062227.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-2026-07-01T04-11-14-525Z\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-2026-07-01T04-11-14-525Z`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-1782879062227.json`、`output\codex-evidence\package-launch-smoke-1782878975868.json`、`output\delivery-bundles\v15-delivery-bundle-2026-07-01T04-11-14-525Z\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
