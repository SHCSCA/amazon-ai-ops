...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782869995057.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83131064`，SHA-256 `4D0EADE9EFFBBAEA1A08E0C391D31F2AAB574589116F2EC39D7AF61F174900F4`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82965311`，SHA-256 `72945B0BDD7757727449DF35F4BF9AD7A1F94E23949D88028D79333D7298F04E` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782875709612.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260701111508.json` 为 2026-07-01 high-fidelity UI / AI/import feedback / data-import row-fade refresh / credential refresh / 产品配置载入编辑行确认反馈 / 产品配置批量目标 ACOS / 产品配置批量选择计数进度 / 数据采集动作按钮 busy 反馈 / Listing 本地动作按钮 busy 反馈 / 定时任务控制器按钮 busy 反馈 / 广告量化指标聚焦降噪与 AI 运行雷达反馈 / 今日看板主行动反馈 / 执行回读安全复选框确认反馈 / 执行回读证据链动作按钮 busy 反馈 / 执行回读本地路径打开 busy 反馈、关键词机会排序与筛选反馈 / ProgressiveDetails 折叠反馈 / 全局字体栈和表格行高契约后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260701111508.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-20260701111508.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260701111508-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260701111508-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
...
| `output/codex-evidence/current-business-ui-smoke-1782869995057.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案、数据采集文案、scheduler 控制器 busy 反馈、关键词机会刷新按钮 busy 反馈和本地路径打开 busy 反馈一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260701111508.json`、`output\codex-evidence\package-launch-smoke-1782875709612.json`、`output\delivery-bundles\v15-delivery-bundle-20260701111508-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
pnpm run write:v15-evidence-manifest --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260701111508.json
pnpm run verify:v15-delivery
pnpm run verify:v15-final-readiness --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260701111508.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782875709612.json --out output\codex-evidence\final-readiness-20260701111508.json
# 先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，再导出交付包；导出器会拒绝 IN_PROGRESS README。
pnpm run export:v15-delivery-bundle --final-readiness output\codex-evidence\final-readiness-20260701111508.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260701111508-ready
pnpm run verify:v15-ready-safety --final-readiness output\codex-evidence\final-readiness-20260701111508.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260701111508-ready\delivery-bundle-manifest.json
```
