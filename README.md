...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1782373770817.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83106997`，SHA-256 `D0FB71496A9DBAD13E626A3700F0B1196007D42182D904F788ABF1819224FF5F`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82941248`，SHA-256 `0CE439FC5E47D22ED1F44B1C68D75DD29D960D978C688661E524FBD81CB3AC1F` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1782373864685.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 当前通过 | `output\codex-evidence\final-readiness-20260625155133.json` 为 2026-06-25 high-fidelity UI / AI/import feedback / credential refresh 后的 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260625155133.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
| READY 安全门 | 通过 | `pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260625155133.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260625155133-ready\delivery-bundle-manifest.json` |
| 交付证据包 | 当前 READY | `output\delivery-bundles\v15-delivery-bundle-20260625155133-ready`；历史 `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` 只作基线参考 |
| 交付证据脚本 | 通过 | 当前应用内执行回读 smoke 覆盖 session 创建、审批盖章反馈、结构检查和中文现场证据未填写提示、打开 `session-input.json` 填写文件、打开 `session-input-guide.md` 填写说明、回读证据生成、回读证据 verifier 和路径展示：`output\codex-evidence\business-ui-ad-execution-smoke-1782373750223.json`；`交付验收` 页新增 `刷新最终验收` 和 `广告回读补证`，可从失败 readback gate 直接创建工作包、检查工作包、显示 `结构通过，现场证据待填写` 和中文缺失字段、生成回读证据、校验回读证据、用该证据刷新最终验收，并打开候选证据/工作包/操作清单/Ads UI 定位单/填写文件/填写说明，settings/delivery smoke 已覆盖：`output\codex-evidence\business-ui-settings-delivery-smoke-1782373762864.json`；当前 final-readiness、bundle export 和 READY safety 已刷新 |
...
| `output/codex-evidence/current-business-ui-smoke-1782373770817.json` | 当前业务 UI 汇总 smoke；覆盖 shell、data pipeline、ad execution、keyword/listing、settings/delivery，证明当前 renderer/operator flow 与新 AI fallback、Listing 草案和数据采集文案一致；并已与本轮 package smoke/final-readiness 刷新同步 |
...
| `output/codex-evidence/business-ui-ad-execution-smoke-1782373750223.json` | 当前广告执行 smoke；覆盖优化建议、审批/回读路径、审批盖章反馈、AI fallback 状态、证据不足阻断、fail-closed 执行边界，以及执行回读页可见的 session 工作包创建、结构检查、中文现场证据未填写提示、打开填写文件、打开填写说明、应用内生成回读证据、路径展示和隐藏技术区的 `prepare/verify/fill session` 命令入口 |
...
| `output/codex-evidence/business-ui-settings-delivery-smoke-1782373762864.json` | 当前设置/交付 smoke；覆盖 AI 设置保存/测试/清除本地 Key、AI 调用审计、交付矩阵、非 READY 展示、`交付验收` 页应用内刷新最终验收，以及从失败 readback gate 创建/检查工作包、显示中文现场证据未填写字段、生成/校验回读证据、用生成的 PASS JSON 刷新最终验收，并打开候选证据、工作包目录、操作清单、Ads UI 定位单、填写文件和填写说明 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260625155133.json`、`output\codex-evidence\package-launch-smoke-1782373864685.json`、`output\delivery-bundles\v15-delivery-bundle-20260625155133-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
...
pnpm run write:v15-evidence-manifest -- --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260625155133.json
pnpm run verify:v15-delivery
pnpm run verify:v15-final-readiness -- --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260625155133.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782373864685.json --out output\codex-evidence\final-readiness-20260625155133.json
# 先把 README 顶部 DELIVERY 行切到当前证据对应的 `APP_READY`，再导出交付包；导出器会拒绝 IN_PROGRESS README。
pnpm run export:v15-delivery-bundle -- --final-readiness output\codex-evidence\final-readiness-20260625155133.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260625155133-ready
pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260625155133.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260625155133-ready\delivery-bundle-manifest.json
```
