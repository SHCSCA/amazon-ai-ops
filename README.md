...
**DELIVERY: APP_READY.** 当前 Windows 桌面版已完成本轮 UX 降噪、主流程业务文案替换、Windows installer/portable 重新打包、packaged launch smoke、manifest-driven final-readiness 和 READY safety。当前权威 final-readiness 为 `output\codex-evidence\final-readiness-20260618170712.json`，并已把 `output\codex-evidence\package-launch-smoke-1781772408989.json` 作为硬门槛，覆盖 `win-unpacked` 与 no-install portable 启动。旧的 `output\codex-evidence\final-readiness-2026-06-18-portable-fix.json` 只保留为历史 READY 证据。应用内广告执行仍保持 fail-closed，不做批量自动写入；后续每个广告动作必须绑定自己的店铺、站点、广告组合、广告活动、广告组、ASIN、对象和动作，并独立审批、截图和回读。

| 项目 | 状态 | 证据/位置 |
|---|---:|---|
| v1.5 基线合并 | 已完成 | v1.5 业务后台重构和最终验收已合入 `master` |
| 本轮收尾改动 | 已完成 | UI 密度、长页面、业务化文案、无安装版启动 smoke、package rebuild hardening 和当前 APP_READY 聚合已接入 |
...
| 当前业务 UI 冒烟 | 通过 | `pnpm run smoke:business-ui-current` 通过，汇总证据 `output\codex-evidence\current-business-ui-smoke-1781771858207.json` |
| 当前安装包证据 | 已进入 final-readiness | installer `apps\desktop\release\AmazonAIOpsAgent-1.5.0.exe`，大小 `83042664`，SHA-256 `E6890C9043BE8140900ACCF146898DD45B841C33FF5F4B3D4301CF5A1BFC94A2`；portable `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`，大小 `82876915`，SHA-256 `D9C181C09B32A1FBC5095281E1DBB788C1C8B9FAE36FA81F31B70E5BF319D1D2` |
| 当前安装/免安装启动复验 | 通过 | `pnpm run smoke:package-launch` 通过，证据 `output\codex-evidence\package-launch-smoke-1781772408989.json`；`win-unpacked` 到达 `ipc-ready/window-created`，portable 从 Temp 解压并启动子进程 |
...
| 最终就绪聚合门 | 通过 | `output\codex-evidence\final-readiness-20260618170712.json` 为 manifest-driven `APP_READY`，manifest 为 `output\codex-evidence\v15-final-readiness-evidence-manifest-20260618170712.json`，包含 `Release package hash` 与 `Package launch smoke` 两个安装包门槛 |
...
| 2 | 复核当前 READY 证据 | `output\codex-evidence\final-readiness-20260618170712.json`、`output\codex-evidence\package-launch-smoke-1781772408989.json`、`output\delivery-bundles\v15-delivery-bundle-20260618170712-ready\delivery-bundle-manifest.json` 三者必须同属当前 package hash |
