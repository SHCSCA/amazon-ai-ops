...
- 当前正式应用候选卡裸露阻断码的源码缺口已完成精确中文映射与红→绿，并进入 app content `08390336...4F3A` 的新 Windows 包；底层安全码、审批门不变。尚待当前 Package UI 通过 manifest 提供新包实屏证据，确认前不得复用旧截图。
- “范围已保存”重挂载反馈缺口已完成红→绿并通过该页 11/11 与 desktop typecheck，不再列为 blocker；尚需进入下一次 Windows 重建并做一次实包回读。

## 当前：2026-08-25 Package UI 仅待新的可见手输提交

- 当前新包已更新为 EXE `67DC2A...5E89` / app content `08390336...4F3A`，Windows 七步、7 类业务 smoke 与 folder ZIP 启动均通过；旧 `-79` 及更早 run group 因 app content 不同已不可续跑。
- 全新 `operator-core-20260825-80` 已绑定该哈希、当前正式库和合规隔离 Profile；首次 60 秒内没有操作者提交，runs=0 后安全关闭。官方 inspector 已返回 `RESUME_SAFE / violations=[]` 并生成一次性 resume receipt；恢复动作仅为操作者确认在窗口前后续跑 `-80` 并亲自手输一次，不重建包、不新建 Profile、不重复前置测试。
- `operator-core-20260825-78` 未进入登录判定即因 runner 启动导航竞态失败，runs=0；这不是账号、ERP 或 Ads 失败。竞态已完成定点红→绿，失败 run group 不复用。
- 恢复动作：使用已生成的一次性 receipt 续跑 `-80`，确认目标应用窗口稳定后由操作者手输一次。执行者不读取、不代填密码，不操控其他应用。
...
- `-79` 已两次稳定显示目标登录页：第一次 300 秒、第二次按用户节奏 60 秒均未检测到提交，第二次已主动停止。该项只缺操作者实际提交，不再阻塞最新采集与 AI 验证；下次恢复前先由操作者确认已看到目标登录窗。
