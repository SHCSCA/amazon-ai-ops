...
- 交付提交 `91158052` 及后续文档提交已保留完整历史；此前两次推送曾因 GitHub `Recv failure: Connection was reset` 失败，随后 HTTP/1.1 推送成功，远端当前为 `09114265`。未改 remote、未强推。
- folder ZIP smoke `output/codex-evidence/folder-zip-launch-smoke-1787114238805.json` 已随 `09114265` 的文档链同步到远端引用；构建产物本身仍按规则不进 Git。
