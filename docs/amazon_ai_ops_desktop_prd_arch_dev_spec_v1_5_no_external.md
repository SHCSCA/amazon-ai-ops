...
## 2026-08-26 v1.5.1 实现修订

- `UNKNOWN` 不再只有阻断提示：Main 对同一当前店铺广告对象执行刷新前后两次只读观察与截图，校验稳定身份后判断目标值、原值、漂移或仍未知；不重试写入，原执行记录继续保持 `unknown`，对账结果另记为 `READBACK`。
- 因果记忆索引从当前店铺的追加式事件真实重建，搜索使用该索引，并可下载含 US/USD 范围、业务日期、记录和检索词的 JSON 时间线；跨店铺事件直接阻断。
- Mission 与经营实验只支持归档/恢复，不提供不可恢复硬删除。串行执行遇 `UNKNOWN` 即停止，不提供“跳过”；唯一真实急停位于策略运行时。
- 工作区可用性按全部保留能力判断：任一真实能力 `BLOCKED` 仍显示“部分可用”，只有全部能力均为原生或受控生产适配时才显示可用。
- 本修订源码单元门为 `285/285` files、`3529/3529` tests passed。`1.5.1` 尚未完成 Windows 动态验收，状态仍为 `APP_NEEDS_WORK / NON_READY`。
