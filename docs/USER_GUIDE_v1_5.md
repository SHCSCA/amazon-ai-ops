...
Open `数据` -> `工作范围` before starting a new analysis pass or when you suspect the current data does not match the ERP view.
...
8. Use the next-step action suggested by the page: no reports routes to `数据采集`, downloaded-but-unimported reports route to `导入校验`, and imported metrics route to `广告表现`.
...
1. Start from the first-screen `产品作战台` task panel. It shows whether a product is locked, how many imported metric rows exist, how many daily rows are available, and whether the next safe action is `补齐成本目标`, `先导入广告指标`, or `进入 AI 量化`.
...
3. Check `当前产品范围`. When a product is selected, `广告表现`, `优化建议`, `运营事件`, `关键词机会`, and `Listing草案` all use that ASIN context.
...
Open `数据` -> `成本目标` when the selected product needs cost, minimum price, margin, ACOS, or TACOS thresholds.
...
Open `数据` -> `广告表现` after the current scope has real Lingxing reports and imported DB metrics.
...
Open `数据` -> `数据采集` from the left menu.
...
The README may list the current `APP_READY` package paths, hashes, final-readiness JSON, package-launch smoke, and delivery bundle. Treat those tables as a human-readable summary. The authoritative release facts are the manifest-driven final readiness JSON, the current package index recorded by final readiness, the delivery bundle manifest, and the READY safety result.
...
```powershell
pnpm run write:v15-evidence-manifest --ad-readback output\codex-evidenceeal-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence15-final-readiness-evidence-manifest-1783043003005.json
```
...
```powershell
pnpm run export:v15-delivery-bundle --final-readiness output\codex-evidenceinal-readiness-1783043003005.json --data-reconciliation output\codex-evidenceeal-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidenceeal-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles15-delivery-bundle-2026-07-03T01-43-56-prototype-parity
```

After the bundle is exported, run the READY safety gate:

```powershell
pnpm run verify:v15-ready-safety --final-readiness output\codex-evidenceinal-readiness-1783043003005.json --ui-smoke output\codex-evidence\current-business-ui-smoke-1783042927932.json --bundle-manifest output\delivery-bundles15-delivery-bundle-2026-07-03T01-43-56-prototype-parity\delivery-bundle-manifest.json
```
...
On `广告` -> `结果核对`, screenshots can be dropped or pasted into approval, before, after, and reload/readback capture targets. The shortcut only reduces path handling; it does not bypass approval, source-row traceability, timestamp order, distinct screenshot, or `verify:ad-readback` requirements.
