...
3. Use the split left menu for the upgraded v1.5 workflow:
   ...
   - `数据与量化` -> `数据导入与校验`: import downloaded/local Lingxing spreadsheets into SQLite, check real-report coverage, and sort the 8-report validation table by report, real file, type, size, SHA-256 checksum, imported rows, or status with `aria-live` feedback. During an import write, the table enters read-only lock: sort headers and row open-file buttons disable until SQLite refresh completes.
   - `数据与量化` -> `运营事件`: local notes for events such as discounts, BD, promotions, deals, coupons, price/stock changes, or Listing changes that affect daily advertising interpretation. The page exposes recording as the first-screen task and highlights the newest saved card.
   ...
   - `广告执行` -> `优化建议`: rules plus AI candidate generation and explanation for ad changes; approval-ready rows can be batch-selected into approval, but approval and execution are intentionally separated from generation.
   ...
   - `关键词与 Listing` -> `关键词机会`: Search Term / SQP / keyword imports, ad-context-split opportunity analysis, filters, evidence columns, sortable virtual-table headers with rotating arrow feedback, and a 100ms filter-result crossfade with `aria-live` count feedback.
   - `关键词与 Listing` -> `Listing 优化`: manual Listing entry and version history as the primary source, optional Lingxing read-only fill, keyword heatmap coverage, draft diff chips, generation skeleton feedback, character-limit warnings, suggestions, accepted-only drafts, and exports.
...
Current 2026-06-26 no-install executable for validation: `apps\desktop\release\AmazonAIOpsAgent-1.5.0-portable.exe`, SHA-256 `F04F32879B70DA7A5BE16D2D0AEE8F1D60937D03A17AE8F9237E1320B724F3C2`. Launch smoke evidence: `output\codex-evidence\package-launch-smoke-1782459094815.json`. This build includes encrypted local remember-account/password support, first-viewport AI/import feedback, approval decision stamp feedback, OperatorTaskPanel action loading/spinner feedback, OperatorTaskPanel shimmer sweep, global button active scale feedback, state-light hover lift feedback, MicroStepper status indicator feedback, operation-scope first-screen confirmation feedback and non-layout-shifting ScopeBar editor popover, product-management first-screen product-locking and credential sandbox hover feedback, product-config first-screen task, keyboard nudge, live target health chips, and inline autosave feedback, operation-event first-screen task feedback, settings threshold field-level validation feedback, scheduler first-screen local task feedback, data-collection monitor drawer with Canvas browser preview, delivery readback repair handoff, ad-quant metric focus filters, canonical daily ad metric explanation, ad readback screenshot paste/drop capture with drag-over feedback, readback time/value contract cards, heavy table virtualization with row press/focus feedback plus sortable header arrow/ARIA and 100ms filter-result feedback for `关键词机会`, heavy table virtualization plus sortable report/file/type/size/SHA-256/row/status headers, import-time read-only table lock, `aria-live` summary, and 100ms blur refresh for `数据导入与校验`, Listing keyword heatmap coverage with draft diff chips, generation skeleton feedback, and over-limit character alarms, the Lingxing report creation fix that commits the date range picker after filling the start/end dates, the product-level workbench refresh that requires an explicit product before ASIN-specific analysis, and the live ad strategy diagnosis JSON-contract fix verified by `output\codex-evidence\ad-strategy-live-1782358641101.json`. In `审批中心`, hover or keyboard-focus one of the three decision buttons to keep that choice prominent while the other available choices fade; in `产品管理`, hover or keyboard-focus `凭证映射通过` to inspect the Main sandbox channel summary, which intentionally shows only the sandbox ID, site/period, and no-plaintext-retention statement.
...
2. Click `刷新调度状态` to reread the task list from the main process. The button enters the shared busy state while the scheduler status is loading.
...
4. Click `执行本地任务` only after checking the named task and purpose. This can download reports, import local data, or generate a local pending recommendation pool, but it does not approve suggestions, change bids, add negatives, pause ads, or write Amazon Ads.
5. Read the fixed `aria-live` feedback line after refresh, confirmation, success, or failure. It keeps scheduler errors visible instead of hiding them in the lower table.
...
4. Use `编辑范围` to open the sticky ScopeBar editor popover from this page, instead of hunting for the top bar control.
5. When editing the ScopeBar fields, each changed date, store, marketplace, ASIN, or batch field flashes a small green confirmation line in its own reserved space. This confirms the value has been recorded in the draft scope without moving nearby controls or pushing the workspace down.
6. Use the next-step action suggested by the page: no reports routes to `数据采集`, downloaded-but-unimported reports route to `数据导入与校验`, and imported metrics route to `广告量化`.
...
2. Review `产品列表` and select the product you want to work on. The page no longer silently selects the first product; the selected product writes its ASIN into the current `scope.asin`.
3. Check `当前产品范围`. When a product is selected, `广告量化`, `优化建议`, `运营事件`, `关键词机会`, and `Listing 优化` all use that ASIN context.
4. Use `产品信息维护` to edit ASIN, title, MSKU/SKU, stage, status, purchase/FBA/min-price fields, target ACOS/TACOS, and target margin. Saving updates the local product configuration, switches the shared workbench to that ASIN, and writes saving/saved/error feedback into the task panel's fixed status line.
...
4. The bottom `保存完整产品配置` action remains available for saving the whole form after changing base product fields such as title, SKU, stage, or status.
...
3. After a chip is clicked, read the fixed focus line. It shows `visible/total` and confirms that the focus only changes the current view.
4. Review `产品/广告对象阶段时间线`, `主要问题摘要`, `复核队列`, and `实体诊断`; these sections follow the selected focus so the operator can inspect one risk dimension at a time.
5. The focus filter does not create recommendations, approve rows, execute Ads changes, or write Amazon Ads. Use `优化建议`, `审批中心`, and `执行回读` for those separate safety steps.
...
2. Click `记录事件` to save the current form through the task panel. While saving, the main action is disabled and shows the running state.
3. The lower form button is labeled `保存到上下文`; use it when you are already editing the form area and do not need the top task action.
4. After saving, the newest event card flashes briefly so the operator can see that the local context record was actually written. This still only updates local context; it does not modify Amazon Ads or Listing content.
...
7. Review whether the page model matched expected entry hints, report names, and selectors.
8. It is recommended to click `导出证据包` after a diagnostic run to create a local verification bundle under the app export directory. The bundle is loaded from the persisted diagnostic ID and contains the diagnostic JSON, active page model, readiness result, selector candidates, action selector checks, screenshot/DOM evidence copies, and a manual verification checklist.
9. Click `生成模型草稿` after a diagnostic run when you want a safe local page-model draft. The draft is filled into the editor and exported with selector candidates and solidification notes, but it deliberately keeps `requiresManualVerification: true`.
10. Use `导出启用审计` before turning off `requiresManualVerification`. The audit checks whether the saved page model has complete scoped action selectors, recent same-model/same-date/same-store/same-marketplace diagnostic setup evidence, and local screenshot/DOM evidence files.
11. After a fresh same-scope diagnostic exists, choose one report type and click `单报表验证` to run a controlled canary through create, wait-ready, download, file verification, and batch evidence for exactly one report. This canary is the proof step for live ready/download selectors before attempting all 8 reports.
12. Click `启动采集`.
13. The app creates a collection batch and records all 8 report types:
   - 广告活动报告
   - 广告组报告
   - 广告位报告
   - 广告（推广的商品）报告
...
On `广告执行` -> `优化建议`, click the first-screen status buckets to switch the table between `全部建议`, `高风险强阻断`, `需人工复核`, and `已就绪可批准`. This is a view filter only; switching buckets clears current checkbox selection so hidden rows cannot be batch-submitted. Use the checkbox column to select only recommendations that pass the formal approval precheck. The `批量提交 X 项到审批中心` action stores a local handoff hint and opens `审批中心`; it does not approve, reject, execute, or write any Ads change. Review-only or evidence-blocked rows stay disabled until their missing source, scope, or AI evidence details are resolved.

On `广告执行` -> `审批中心`, approval and rejection now provide first-screen stamp feedback. `SEALING` means the app is recording the approval request, `PASSED` means the local recommendation record was approved, `REJECTED` means the recommendation was explicitly blocked, and `BLOCKED` means a required approver, reason, target, source, or safety precheck is missing. When arriving from a batch handoff, the page shows a `批量送审` hint and selects the first matching row, but it still reloads the real approval queue and requires each row to pass the same evidence and safety checks. These stamps are only decision feedback. They do not execute Amazon Ads changes. A `PASSED` decision must still continue to `执行回读` for manual Ads UI action, before/after screenshots, reload/readback evidence, and `verify:ad-readback`.

## Final Readiness Evidence

Use `交付验收` to review the delivery state. As of the 2026-06-26 high-fidelity Windows UI, AI output-contract refresh, keyword-opportunity sortable header and filter-axis feedback, real AI strategy JSON-contract fix, encrypted credential save, AI/import feedback refresh, OperatorTaskPanel loading feedback, OperatorTaskPanel shimmer sweep, global button active scale feedback, settings threshold field-level validation feedback, shared FormTable focus-within glow feedback, Lingxing report date-picker fix, data-collection monitor drawer with Canvas browser preview, delivery readback repair handoff with field-level red repair rings, ad readback screenshot capture with drag-over feedback, readback time/value contract visualization, Listing keyword heatmap matrix plus draft diff/skeleton/limit feedback, canonical daily metric explanation, and product-level workbench refresh, the packaged state is `APP_READY` for the evidence set fixed by `output\codex-evidence\v15-final-readiness-evidence-manifest-20260626153203.json`, `output\codex-evidence\final-readiness-20260626153203.json`, `output\codex-evidence\package-launch-smoke-1782459094815.json`, and `output\delivery-bundles\v15-delivery-bundle-20260626153203-ready`. This state includes the AI contract tags, compact metric/tag views, business-domain navigation with active glow bar, product maintenance, per-product daily ad metrics, table-like Listing editor, Listing keyword heatmap, Listing draft red/green diff chips, generation skeleton wave, over-limit character alarms, sortable keyword-opportunity virtual-table headers, data-import read-only import lock, and filter-axis feedback, field-level threshold repair prompts and focus-within row glow in shared FormTable surfaces, global enabled-button active scale feedback, shared state-light hover lift feedback, data-collection monitor drawer with Canvas browser preview, structured output-token floor, OpenAI-compatible saved temperature/maxTokens usage, strategy-diagnosis evidence-ref normalization, concrete JSON output examples, automatic commit/close behavior for the Lingxing date range picker during report creation, encrypted local credential save, first-viewport AI/import status feedback, screenshot drag/drop or Ctrl+V capture on the readback page with `松开即可存证` drag-over feedback, visible readback time/value contract cards, and explicit canonical daily ad metric accounting. Live ad strategy diagnosis evidence `output\codex-evidence\ad-strategy-live-1782358641101.json` returned `source=ai` with no fallback on the current DB scope. Historical evidence `output\codex-evidence\final-readiness-2026-06-18-portable-fix.json` and bundle `output\delivery-bundles\v15-delivery-bundle-2026-06-18-portable-fix-ready` remain useful as baseline proof only. The verified current-contract ad sample was a paused FT-US keyword `door lock` bid decrease from live `1.30` to `1.17`; the source recommendation remained traceable as `1.63 -> 1.46`, but was not written because the live bid was already lower than the source recommendation. Future ad changes must not reuse that scope and must each provide their own target, source report files/row, approval, before/after screenshots, and readback evidence.
...
```powershell
pnpm run write:v15-evidence-manifest -- --ad-readback output\codex-evidence\real-ad-execution-readback-candidate-rec-4-current-pass.json --out output\codex-evidence\v15-final-readiness-evidence-manifest-20260626153203.json
```
...
```powershell
pnpm run verify:v15-final-readiness -- --evidence-manifest output\codex-evidence\v15-final-readiness-evidence-manifest-20260626153203.json --package-launch-smoke output\codex-evidence\package-launch-smoke-1782459094815.json --out output\codex-evidence\final-readiness-20260626153203.json
```
...
```powershell
pnpm run export:v15-delivery-bundle -- --final-readiness output\codex-evidence\final-readiness-20260626153203.json --data-reconciliation output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.json --data-reconciliation-md output\codex-evidence\real-lingxing-reconciliation-batch_20260612020905629_gkchz1.md --out output\delivery-bundles\v15-delivery-bundle-20260626153203-ready
```

After the bundle is exported, run the READY safety gate:

```powershell
pnpm run verify:v15-ready-safety -- --final-readiness output\codex-evidence\final-readiness-20260626153203.json --bundle-manifest output\delivery-bundles\v15-delivery-bundle-20260626153203-ready\delivery-bundle-manifest.json
```
