...
1. Open Amazon AI Ops Agent. The app enters the local operating system directly; there is no global ERP/Ads login gate. If no store is selected, the main shell keeps store-scoped business actions paused and opens the left-side store selector instead of silently choosing a store.
2. Use the left-side `店铺与站点` surface to create or maintain a store. Store name and local identity are configurable; marketplace and currency are fixed to US / USD in this release. Creating a store does not select it and does not start a browser Profile or job. Click `切换店铺` only after checking the target store. Archived stores can be restored, but cannot be used for collection or execution while archived.
3. In `店铺与广告对象` → `店铺与产品`, configure the selected store's Lingxing username and the exact store name shown in Lingxing Download Center. Enter the Lingxing password only when starting or refreshing that store's visible browser connection. Passwords are consumed by Electron Main and, only when explicitly requested, stored through the local encrypted credential boundary; they are never returned to the Renderer as saved plaintext.
4. Click `启动当前店铺连接`. The visible store-isolated Chromium handles Lingxing ERP login, verification/MFA and the Lingxing Ads authorization flow. The operator does not enter an Ads Profile ID. Electron Main accepts a stable advertising identity only from trusted `https://ads.lingxing.com` URL/active-page evidence. If the current page does not expose unique evidence, open a campaign or ad-group page in the visible Ads window.
5. When the workbench shows an automatically detected advertising account, return to the main window and click `确认绑定到当前店铺`. Main re-reads the currently visible identity and rejects an expired token, another store, another browser runtime, or an account switch. Until confirmation succeeds, real advertising execution remains blocked. Missing Lingxing blocks collection for that store; missing/unconfirmed Ads identity blocks real advertising execution, but neither condition hides the rest of the local system.
6. Use the 10 visible workspaces in the left menu. Each workspace keeps the active store visible and never mixes another store's facts or browser Profile:
...
The current version is `1.5.1` and remains `APP_NEEDS_WORK / INTERNAL NON_READY`, not `APP_READY`. A static Windows candidate exists, but it has no current runtime acceptance lineage under the operator's dynamic-test prohibition.

Policy scope and execution authority now use real current-store evidence rather than names. A keyword scope includes its match type and exact imported source row; campaign and ad-group scopes use canonical Ads identities internally while ordinary screens show readable paths. Policy create/update/enable and MissionGrant issuance recheck the current import, evidence revisions and current Ads session. Changing store, batch, job or grant invalidates pending execution results.

Current verification is `157/157` focused authority tests, `285/285` files and `3584/3584` allowed unit tests, plus `12/12` package-script contracts, with no committed skip/todo declarations. Static `pnpm run build:win` passed seven steps and produced the 1.5.1 installer, portable executable and folder ZIP. Typecheck, business smoke, Package UI, folder-ZIP launch, current-package formal-DB pre/post verification and real Ads validation were not run because the operator prohibited dynamic testing; the historical `1.5.0` manifest does not authorize this package.

Task 8B remains fail-closed. First perform a current read-only object/bid check and require a current `lower_bid` within 10%. Then show the operator the exact path, current value, target value, change percentage, evidence date and object revision; only that specific approval may authorize one real write and reload readback. No Ads write occurred in this round.
...
The final regression is `output\codex-evidence\full-vitest-external-security-p1-20260717-final.json` (584/584 suites, 1992/1992 tests). Package launch passed at `output\codex-evidence\package-launch-smoke-1784276358829.json`; package security passed 11/11 at `output\codex-evidence\package-security-boundaries-20260717-p1.json`; business smoke passed at `output\codex-evidence\current-business-ui-smoke-1784276952256.json`; and package UI schema v5 passed at `output\codex-evidence\package-ui-evidence-20260717-p1\2026-07-17T08-21-12-482Z\manifest.json` for 100%/125% (8 workspaces + 3 overlays each) plus the wide Product/Diagnosis profile. Its historical identity is installer `A08715C80D660DDA615324FC146A164C5D3C19232BE6E55E90859348C9C01637`, portable `E8961E89B53A19F1C11D9A0DAFCC1797B0DE7C90B7972196B52D0F9F062FE1FE`, win-unpacked EXE `67DC2A7036860A68E5312C212C31B8772AC463ED0289FCC44897867F55075E89`, app content `8A9132109B9C2C6A4C1AA6A1EB18EFC675E53403004CF7000CC6C2A5C01AFF34`, and main bundle `74046AD904EE2DFFB77E892367F7D38E0BD695F89A5F7A88BE6EF97A848035B9`. Historical final readiness was `output\codex-evidence\final-readiness-20260717-external-security-p1-non-ready.json` (`APP_NEEDS_WORK`, 7/8).

At that historical snapshot Task 8B was the only external completion blocker. It required a current positive recommendation and real human approval, distinct Amazon Ads v2 before/after/reload screenshots, and the correct authority-DB recommendationId. This historical statement does not remove the current 2026-08-20 full-8 import blocker. The product-management page does not show a `凭证映射通过` or `Main Sandboxed ID` chip; credential status belongs to the login surface.
...
| Portable SHA-256 | `CE41FCF95EF592B839CFF3660B1C9DB11A2F546FB7F38D0CDD7160CA27E51B48` |
...
| Package UI | `output\codex-evidence\package-ui-evidence\2026-07-17T06-33-01-390Z\manifest.json` (schema v5, 30 PNG, per-run isolation and diagnostics passed) |
| Workspace UI | `output\codex-evidence\workspace-ui-task6\workspace-ui-evidence-run-2026-07-17T06-23-26-823Z.json` (46/46, including 3 DEV preview state targets) |
...
| Package launch smoke | `output\codex-evidence\package-launch-smoke-1784269772321.json` |
| Evidence selection | `output\codex-evidence\v15-final-readiness-evidence-manifest-20260717-ui-p2-non-ready.json` |
| Final readiness | `output\codex-evidence\final-readiness-20260717-ui-p2-non-ready.json` (`APP_NEEDS_WORK`, 7/8) |
...
| NON_READY bundle | `output\delivery-bundles\v15-delivery-bundle-20260717-ui-p2-non-ready\delivery-bundle-manifest.json` (`APP_NEEDS_WORK`; exported; strict `verify:v15-non-ready-safety` passed 17/17) |

That superseded package was for internal validation and controlled handoff only. Its navigation/redirect, external-link, and legacy-credential P1 findings are now closed by the current candidate. Task 8B remains estimated at 30–60 minutes once a current positive recommendation, human approval, and Ads access exist. Eval/history cleanup remains non-blocking.

Superseded package UI runs from 2026-07-15/16, the pre-shutdown-fix `2026-07-17T04-02-25-468Z` run, and the Task 8A `2026-07-17T04-16-32-110Z` run are retained only to explain the repair sequence. They are not current delivery proof.
...
The historical 2026-07 external-security candidate was `output\codex-evidence\final-readiness-20260717-external-security-p1-non-ready.json`: `APP_NEEDS_WORK`, 7/8, with only Task 8B `real-ad-execution-readback` blocked at that time. Its launch smoke, schema-v5 package UI, package-security evidence, regression and NON_READY bundle remain historical records only. READY and NON_READY safety bind three package identities: win-unpacked EXE, app content, and `dist/main/index.js`; none of those old identities grants current delivery credit.

#### Superseded UI P2 release summary

At the superseded UI P2 snapshot, package UI `output\codex-evidence\package-ui-evidence\2026-07-17T06-33-01-390Z\manifest.json`, smoke `output\codex-evidence\package-launch-smoke-1784269772321.json`, evidence selection `output\codex-evidence\v15-final-readiness-evidence-manifest-20260717-ui-p2-non-ready.json`, and final readiness `output\codex-evidence\final-readiness-20260717-ui-p2-non-ready.json` referred to the same then-current package candidate. The isolated schema-v5 package UI run recorded 30 PNGs across 100%/125% compact matrices plus a 1400×900 Product/Diagnosis wide profile and proved per-run DB/process/profile-browser/diagnostics isolation. Its NON_READY bundle manifest at `output\delivery-bundles\v15-delivery-bundle-20260717-ui-p2-non-ready\delivery-bundle-manifest.json` recorded `APP_NEEDS_WORK`; export succeeded and strict `verify:v15-non-ready-safety` passed 17/17. Those facts are historical only and cannot be used as current delivery authority. Task 8B was already the only failed formal gate at that snapshot, while three baseline P1 security items were still open; the current external-security candidate in the section above closes those P1 items.
...
pnpm run smoke:package-security-boundaries -- --expected-exe-sha256 <win-unpacked-sha256> --expected-app-content-sha256 <app-content-sha256> --out <current-package-security-evidence.json>
```
...
```powershell
pnpm run export:v15-delivery-bundle -- --final-readiness <current-final-readiness.json> --package-ui-manifest <current-package-ui-manifest.json> --package-security-evidence <current-package-security-evidence.json> --workspace-ui-manifest <current-workspace-ui-manifest.json> --business-ui-smoke <current-business-ui-smoke.json> --full-test-evidence <full-vitest.json> --data-reconciliation <current-reconciliation.json> --data-reconciliation-md <matching-current-reconciliation.md> --release-dir apps/desktop/release --readme README.md --db <amazon-ai-ops.db> --skip-latest-extras true --out <new-non-ready-bundle>
pnpm run verify:v15-non-ready-safety -- --final-readiness <current-final-readiness.json> --package-launch-smoke <current-package-launch-smoke.json> --package-ui-manifest <current-package-ui-manifest.json> --package-security-evidence <current-package-security-evidence.json> --bundle-manifest <new-non-ready-bundle>/delivery-bundle-manifest.json --readme README.md --db <amazon-ai-ops.db>
```
...
```powershell
pnpm run export:v15-delivery-bundle -- --final-readiness <app-ready-final-readiness.json> --package-ui-manifest <current-package-ui-manifest.json> --package-security-evidence <current-package-security-evidence.json> --workspace-ui-manifest <current-workspace-ui-manifest.json> --business-ui-smoke <current-business-ui-smoke.json> --full-test-evidence <full-vitest.json> --data-reconciliation <reconciliation.json> --data-reconciliation-md <matching-reconciliation.md> --release-dir apps/desktop/release --readme README.md --db <amazon-ai-ops.db> --skip-latest-extras true --out <new-ready-bundle>
pnpm run verify:v15-ready-safety -- --final-readiness <app-ready-final-readiness.json> --ui-smoke <current-business-ui-smoke.json> --bundle-manifest <new-ready-bundle>/delivery-bundle-manifest.json --package-ui-manifest <current-package-ui-manifest.json> --package-security-evidence <current-package-security-evidence.json> --db <amazon-ai-ops.db>
```
