...
3. Use the split left menu for the upgraded v1.5 workflow:
   ...
   - `关键词与 Listing` -> `Listing 优化`: manual Listing entry and version history as the primary source, optional Lingxing read-only fill, local save/read/history-refresh/generate/export button busy feedback, keyword heatmap coverage, the `关键词与本地草案工作台`, draft diff chips, generation skeleton feedback, character-limit warnings, local preview drafts, and exports.
...
5. Use the current-scope product table when several products need the same target ACOS. Select one or more rows, enter `目标 ACOS (%)` such as `35`, and click `应用到 X 个产品`. While the bulk save is running, the button changes to `批量应用中...`, shows a spinner/`aria-busy`, and locks the toolbar inputs until the local save returns. The page converts the percent into the local decimal threshold, saves through `saveProductConfig`, keeps failed rows selected, and reports success or failure in the fixed feedback line.
6. The bulk ACOS toolbar only updates local product configuration. It does not approve recommendations, generate AI advice, execute bid changes, or write Amazon Ads.
7. The bottom `保存完整产品配置` action remains available for saving the whole form after changing base product fields such as title, SKU, stage, or status. During the save it changes to `保存中...`, shows a spinner/`aria-busy`, and locks `补充运营事件` plus `进入广告量化` as plain disabled peers so the running action is unambiguous.
...
7. The focus filter and AI-running feedback do not create recommendations, approve rows, execute Ads changes, or write Amazon Ads. Use `优化建议`, `审批中心`, and `执行回读` for those separate safety steps.
...
The first-screen `复制摘要` action on `交付验收` is a clipboard helper. While it writes the summary, it changes to `复制中...`, shows a spinner, exposes `aria-busy=true`, and locks the peer first-screen action. This only confirms that the clipboard operation is running; it does not change final-readiness evidence or export eligibility.
...
The local open actions on `执行回读` use the same feedback contract. `打开导出文件`, `打开工作包`, `打开填写文件`, and `打开填写说明` switch only the active button to `打开中...`, show a spinner, expose `aria-busy=true`, and lock sibling path buttons while the OS open request is pending. This only confirms that Windows is opening a local file or folder; it does not generate, fill, verify, or approve readback evidence.

The folded backup command buttons on `执行回读` follow the same clipboard contract. `复制创建工作包命令`, `复制检查工作包命令`, `复制生成回读证据命令`, and `复制长参数生成命令` switch only the active copy button to `复制中...`, show a spinner, expose `aria-busy=true`, and lock peer copy buttons until the clipboard write returns. These backup commands are technical convenience only; they do not approve, execute, verify, or write Amazon Ads.
