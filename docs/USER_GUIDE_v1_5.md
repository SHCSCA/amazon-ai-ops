...
The package is for internal validation and controlled handoff only. Independent audit found three external-distribution P1 blockers outside the formal eight readiness gates: a navigation/redirect allowlist, an `openExternal` `http(s)` allowlist, and legacy plaintext-credential migration. The authority DB currently contains an encrypted key and no legacy plaintext key, but the migration path is still required for older installations. The three P1 fixes are estimated at 4–6 focused engineering hours in total. Task 8B is estimated at 30–60 minutes once a current positive recommendation, human approval, and Ads access exist. Eval/history cleanup is non-blocking and estimated at 0.5–1.5 hours.
...
The README lists the current delivery status, package paths, hashes, final-readiness JSON, package-launch smoke, and delivery bundle. Treat those tables as a human-readable summary. The authoritative release facts are the manifest-driven final readiness JSON, its recorded package index, the delivery bundle manifest, and the matching READY or NON_READY safety result. As of 2026-07-17 the current candidate is `APP_NEEDS_WORK`, not `APP_READY`: package UI `output\codex-evidence\package-ui-evidence\2026-07-17T06-33-01-390Z\manifest.json`, smoke `output\codex-evidence\package-launch-smoke-1784269772321.json`, evidence selection `output\codex-evidence\v15-final-readiness-evidence-manifest-20260717-ui-p2-non-ready.json`, and final readiness `output\codex-evidence\final-readiness-20260717-ui-p2-non-ready.json` refer to the same package candidate. The isolated schema-v5 package UI run records 30 PNGs across 100%/125% compact matrices plus a 1400×900 Product/Diagnosis wide profile, and proves per-run DB/process/profile-browser/diagnostics isolation. The NON_READY bundle manifest at `output\delivery-bundles\v15-delivery-bundle-20260717-ui-p2-non-ready\delivery-bundle-manifest.json` records `APP_NEEDS_WORK`; export succeeded and strict `verify:v15-non-ready-safety` passed 17/17. This authorizes an internal NON_READY handoff only, not `APP_READY` or external distribution. Earlier package UI runs, hashes, smoke files, readiness files, bundles, and safety results are historical and cannot be used as current delivery authority. Within the formal eight gates, Task 8B remains blocked only on the new real Ads v2 authority readback; outside those gates, the three baseline P1 security items above must also be closed before external distribution.
