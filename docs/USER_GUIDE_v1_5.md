...
10. After the page model is enabled and a fresh same-scope diagnostic exists, choose one report type and click `单报表验证` to run a controlled canary through the same preflight, create, wait-ready, download, file verification, and evidence path as full collection. Use this before attempting all 8 reports.
11. Click `启动采集`.
