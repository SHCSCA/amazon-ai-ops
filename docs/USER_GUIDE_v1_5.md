...
Task 8B is now the only external completion blocker: use a current positive recommendation and real human approval, capture distinct Amazon Ads v2 before/after/reload screenshots, and bind the result to the correct authority-DB recommendationId. Once approval/access are available, allow 30–60 minutes. Non-blocking P2 follow-ups are a business-domain allowlist beyond the current protocol allowlist (optional 1–2 hours), preventing a reused ERP session from saving a password not verified on a visible login screen, and adding an adversarial-`NODE_ENV` packaged dynamic smoke. The current product-management page does not show a `凭证映射通过` or `Main Sandboxed ID` chip; credential status belongs to the login surface.
...
The authoritative current candidate is `output\codex-evidence\final-readiness-20260717-external-security-p1-non-ready.json`: `APP_NEEDS_WORK`, 7/8, with only Task 8B `real-ad-execution-readback` blocked. Its launch smoke is `output\codex-evidence\package-launch-smoke-1784276358829.json`, package UI is `output\codex-evidence\package-ui-evidence-20260717-p1\2026-07-17T08-21-12-482Z\manifest.json`, package security is `output\codex-evidence\package-security-boundaries-20260717-p1.json` (11/11), and the final regression is `output\codex-evidence\full-vitest-external-security-p1-20260717-final.json` (584 suites / 1992 tests). The matching bundle is `output\delivery-bundles\v15-delivery-bundle-20260717-external-security-p1-non-ready`; it was exported after documentation freeze and passed all 19 strict NON_READY safety checks. READY and NON_READY safety bind three package identities: win-unpacked EXE, app content, and `dist/main/index.js`.
