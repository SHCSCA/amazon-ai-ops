...
On `广告执行` -> `优化建议`, use the checkbox column to select only recommendations that pass the formal approval precheck. The `批量提交 X 项到审批中心` action stores a local handoff hint and opens `审批中心`; it does not approve, reject, execute, or write any Ads change. Review-only or evidence-blocked rows stay disabled until their missing source, scope, or AI evidence details are resolved.
