...
# 独立复核升级库、v8 backup 绑定和 pre-v8 恢复副本。
...
- migration 1–8 全部为 `applied`，升级库 FK 检查为零；
- v8 manifest 绑定升级前版本、Schema 指纹、逐表行数、backup SHA 和完整性；
- 源业务表行数没有减少；
...
验证器生成的 `restored-pre-v8.db` 是恢复演练副本，不会自动替换真实库。需要回滚时：

1. 保持应用退出并再次确认真实源库没有 `-wal`/`-shm`。
2. 保存失败库、升级 manifest、`.pre-upgrade-to-v8.bak` 与 sidecar，不删除现场证据。
