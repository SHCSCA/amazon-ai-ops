...
function model(overrides: Partial<DownloadCenterPageModel> = {}): DownloadCenterPageModel {
  return {
    ...
    actionSelectors: {
      storeSearchInput: '[data-testid="store-search"]',
      storeOption: '[data-testid="store-option"]',
      storeMoveButton: '[data-testid="store-move"]',
      reportSearchInput: '[data-testid="search"]',
      reportTypeSelect: '[data-testid="report-type-select"]',
      reportTypeOption: '[data-testid="report-type-option"]',
      dateStartInput: '[data-testid="start"]',
      dateEndInput: '[data-testid="end"]',
      dailyDetailRadio: '[data-testid="daily-detail"]',
...
function check(name: DownloadCenterActionSelectorCheck['name'], selector: string): DownloadCenterActionSelectorCheck {
  return {
    ...
    required: true,
    kind: name === 'createReportButton' || name === 'confirmCreateButton' ? 'click' : name.includes('Input') ? 'input' : 'optional',
    found: true,
    usable: true,
    ambiguous: false,
    matchCount: 1,
...
function diagnostic(pageModel = model(), overrides: Partial<DownloadCenterDiagnosticResult> = {}): DownloadCenterDiagnosticResult {
  ...
  return {
    ...
    actionSelectorChecks: [
      check('storeSearchInput', selectors.storeSearchInput!),
      check('storeOption', selectors.storeOption!),
      check('storeMoveButton', selectors.storeMoveButton!),
      check('reportSearchInput', selectors.reportSearchInput!),
      check('reportTypeSelect', selectors.reportTypeSelect!),
      check('reportTypeOption', selectors.reportTypeOption!),
      check('dateStartInput', selectors.dateStartInput!),
      check('dateEndInput', selectors.dateEndInput!),
      check('dailyDetailRadio', selectors.dailyDetailRadio!),
      check('createReportButton', selectors.createReportButton),
      check('confirmCreateButton', selectors.confirmCreateButton!),
    ],
