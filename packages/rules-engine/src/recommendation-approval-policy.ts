...
export interface RecommendationApprovalPolicyOptions {
  ...
  /**
   * Authority-specific ownership checks are supplied by the caller. Main uses
   * current database row authority while browser preview uses its locked
   * in-memory report authority. Omitting this adapter is intentionally blocked.
   */
  writableTargetOwnershipBlockers?: readonly string[];
}
...
function sameWritableTarget(
  left: ActionRecommendation['evidence']['writableTarget'],
  right: ActionRecommendation['evidence']['writableTarget'],
): boolean {
...
export function getExecutableValueBlockers(recommendation: ActionRecommendation): string[] {
...
export function getRecommendationApprovalBlockers(
  ...
  if (recommendation.evidence?.writableTarget) {
    if (!options.writableTargetOwnershipBlockers) {
      blockers.push('Ads 可写对象不属于当前建议：缺少当前权威归属校验');
    } else if (options.writableTargetOwnershipBlockers.length > 0) {
      blockers.push(`Ads 可写对象不属于当前建议：${options.writableTargetOwnershipBlockers.join('、')}`);
    }
  }
