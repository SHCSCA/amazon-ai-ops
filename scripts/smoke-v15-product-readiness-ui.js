...
async function main() {
  ...
  await page.getByText('广告运营', { exact: true }).waitFor();
  await page.getByText('关键词与 Listing', { exact: true }).waitFor();
  await page.getByText('交付与系统', { exact: true }).waitFor();
  await page.getByRole('button', { name: '交付验收' }).click();
  ...
  await page.getByRole('button', { name: '广告报表' }).click();
  await page.getByRole('heading', { name: '广告报表采集', level: 2 }).waitFor();
  await page.getByText('围绕店铺、站点和日期范围完成领星广告报表验证').waitFor();
  await page.getByText('拿到当前范围 8 类报表', { exact: true }).waitFor();
  await page.getByText('下载批次 + 诊断证据 + 验收审计', { exact: true }).waitFor();
  await page.getByText('广告报告采集', { exact: true }).waitFor();
  await page.getByText('不等于 DeepSeek AI 草案或广告写操作已通过验收').waitFor();
  await page.getByLabel('广告报表范围预设').waitFor();
  await page.getByText('一键套用已验证 full-8 范围', { exact: false }).waitFor();
  await expectNoOldWorkbench(page);
  await expectAbsent(page, '交付状态：还差 1 项验收');
  await expectAbsent(page, '导入并生成机会');
  await expectAbsent(page, 'Listing 建议');
  await capturePage('reports', '广告报表');
