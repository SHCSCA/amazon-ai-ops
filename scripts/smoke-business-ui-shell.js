...
async function main() {
  ...
  const routes = [
    ...
    { nav: NAV_RE.operationEvents, heading: /运营事件标记/, label: '运营事件', key: 'operation-events' },
    { nav: NAV_RE.adQuant, heading: /量化诊断中心/, label: '广告量化', key: 'ad-quant' },
    { nav: NAV_RE.recommendations, heading: /优化建议草案/, label: '优化建议', key: 'recommendations' },
    { nav: NAV_RE.approval, heading: /审批历史中心/, label: '审批中心', key: 'approval' },
    { nav: NAV_RE.readback, heading: /渐进执行回读/, label: '执行回读', key: 'readback' },
    { nav: NAV_RE.keyword, heading: /关键词机会矩阵/, label: '关键词机会', key: 'keyword-opportunities' },
    { nav: NAV_RE.listing, heading: /Listing 结构重写/, label: 'Listing 优化', key: 'listing-optimization' },
    { nav: NAV_RE.scheduler, heading: /本地定时调度/, label: '定时任务', key: 'scheduler' },
    { nav: NAV_RE.settings, heading: /AI 适配与诊断/, label: '设置', key: 'settings' },
    { nav: NAV_RE.delivery, heading: /最终验收就绪门/, label: '交付验收', key: 'delivery' },
  ];
